...
export const CurrencyConverter = () => {
  ...
  const [convertedAmount, setConverterAmount] = useState(null);
  ...
  const convertCurrency = async () => {
    ...
    try {
      const res = await fetch(
        `https://api.frankfurter.dev/v1/latest?amount=${amount}&base=${fromCurrency}&to=${toCurrency}`
      );
      const data = await res.json();

      setConverterAmount(data.rates[toCurrency] + " " + toCurrency);
    } catch (error) {
      console.error("Error fetching data: ", error);
    } finally {
      setConverting(false);
